...
class AppSubscriptionLimit(models.Model):
    LIMIT_TYPES = [
        ('Limited', 'Limited'),
        ('Unlimited', 'Unlimited'),
    ]
    
    subscription = models.ForeignKey(Subscription, on_delete=models.CASCADE, related_name='app_limits')
    app = models.ForeignKey('app.Apps', on_delete=models.CASCADE)
    limit_type = models.CharField(max_length=10, choices=LIMIT_TYPES, default='Limited')
    ...
    def __str__(self):
        return f"{self.subscription.name} - {self.app.name} ({self.limit_type})"
